...
export function HomeMangalacharan({ lines }: { lines: string[] }) {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="show"
      className="relative mx-auto grid max-w-6xl grid-cols-2 items-start gap-x-4 gap-y-4 px-1 pt-4 pb-2 sm:px-6 min-[900px]:grid-cols-[auto_1fr_auto] min-[900px]:gap-x-8"
    >
      <div className="col-span-2 row-start-1 text-center min-[900px]:col-span-1 min-[900px]:col-start-2 min-[900px]:translate-x-6">
        {lines.map((text) => (
          <motion.p
            key={text}
            variants={line}
            className="animate-text-shimmer font-serif text-sm font-semibold leading-relaxed sm:text-base md:text-lg"
          >
            {text}
          </motion.p>
        ))}
      ...
      >
        ...
        <img
          ...
          className="h-20 w-auto object-contain max-sm:-translate-x-4 sm:h-28 md:h-36"
        />
