  ...
  return (
    <form action={formAction} onSubmit={handleSubmitGuard} className="space-y-10">
      ...
      <Section
        ...
        subtitle={
          isEdit
            ? 'This book is live on the पुस्तकें / Books page.'
            : 'Saving this book publishes it immediately on the पुस्तकें / Books page.'
        }
      >
        <label className="flex items-start gap-3 rounded-xl border border-neutral-200 bg-white p-4 transition-colors hover:border-gold-500/50">
          <input
            type="checkbox"
            name="is_home_pinned"
            defaultChecked={book?.is_home_pinned ?? false}
            className="mt-0.5 h-5 w-5 rounded border-neutral-300 text-crimson-800 focus:ring-crimson-800"
          />
          <span>
            <span className="block text-sm font-medium text-neutral-900">Pin to homepage</span>
            <span className="mt-0.5 block text-xs text-neutral-500">
              Featured in the drag deck on the homepage. Keep this to a curated handful — the full list
              always stays on the पुस्तकें / Books page regardless of this setting.
            </span>
          </span>
        </label>
      </Section>
