...
import { toggleTeachingPublished, deleteTeaching } from '@/lib/actions/teachings';
import type { Database } from '@/types/database';

type Teaching = Database['public']['Tables']['teachings']['Row'];

export function TeachingRow({ teaching }: { teaching: Teaching }) {
  const [isPending, startTransition] = useTransition();
  const [confirming, setConfirming] = useState(false);
  const [removed, setRemoved] = useState(false);
  const [published, setPublished] = useState(teaching.is_published);

  function handleToggle() {
    startTransition(async () => {
      const res = await toggleTeachingPublished(teaching.id);
  ...
  function handleDelete() {
    startTransition(async () => {
      const res = await deleteTeaching(teaching.id);
